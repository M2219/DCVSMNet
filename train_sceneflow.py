...
parser.add_argument(
    "--dataset", default="sceneflow", help="dataset name", choices=__datasets__.keys()
)
parser.add_argument(
    "--datapath", default="/datasets/SceneFlow/flyingthings3d", help="data path"
)
parser.add_argument(
    "--trainlist", default="./filenames/sceneflow_train.txt", help="training list"
)
parser.add_argument(
    "--testlist", default="./filenames/sceneflow_test.txt", help="testing list"
)
...
parser.add_argument(
    "--lrepochs",
    type=str,
    default="20,32,40,48,56:2",
    help="the epochs to decay lr: the downscale rate",
)

parser.add_argument(
    "--logdir", default="", help="the directory to save logs and checkpoints"
)
parser.add_argument(
    "--loadckpt", default="", help="load the weights from a specific checkpoint"
)
...
parser.add_argument(
    "--summary_freq", type=int, default=1, help="the frequency of saving summary"
)
parser.add_argument(
...
def train():
    ...
    for epoch_idx in range(start_epoch, args.epochs):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            print(
                "Epoch {}/{} | Iter {}/{} | train loss = {:.3f}({:.3f}) | EPE = {:.3f}({:.3f}) | D1 = {:.3f}({:.3f}) | time = {:.3f}".format(
                    epoch_idx,
                    args.epochs,
                    batch_idx,
                    len(TrainImgLoader),
        ...
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            print(
                "Epoch {}/{} | Iter {}/{} | test loss = {:.3f}({:.3f}) | EPE = {:.3f}({:.3f}) | D1 = {:.3f}({:.3f}) | time = {:.3f}".format(
                    epoch_idx,
                    args.epochs,
                    batch_idx,
                    len(TestImgLoader),
...
def train_sample(sample, compute_metrics=False):
    model.train()
    imgL, imgR, disp_gt, disp_gt_low = (
        sample["left"],
        sample["right"],
        sample["disparity"],
        sample["disparity_low"],
    )
    imgL = imgL.cuda()
    imgR = imgR.cuda()
    ...
    if compute_metrics:
        with torch.no_grad():
            scalar_outputs["EPE"] = [
                EPE_metric(disp_est, disp_gt, mask) for disp_est in disp_ests_final
            ]
            scalar_outputs["D1"] = [
                D1_metric(disp_est, disp_gt, mask) for disp_est in disp_ests_final
            ]
    loss.backward()
    optimizer.step()
