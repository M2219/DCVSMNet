...
class Scale(object):
    ...
    def __call__(self, inputs, target):
        ...
        inputs[0] = cv2.resize(
            inputs[0], None, fx=ratio, fy=ratio, interpolation=cv2.INTER_CUBIC
        )
        inputs[1] = cv2.resize(
            inputs[1], None, fx=ratio, fy=ratio, interpolation=cv2.INTER_CUBIC
        )
        target = (
...
class RandomVdisp(object):
    ...
    def __call__(self, inputs, target):
        ...
        image_center = (
            np.random.uniform(0, inputs[1].shape[0]),
            np.random.uniform(0, inputs[1].shape[1]),
        )
        rot_mat = cv2.getRotationMatrix2D(image_center, angle2, 1.0)
        inputs[1] = cv2.warpAffine(
            inputs[1], rot_mat, inputs[1].shape[1::-1], flags=cv2.INTER_LINEAR
        )
        trans_mat = np.float32([[1, 0, 0], [0, 1, px2]])
        inputs[1] = cv2.warpAffine(
            inputs[1], trans_mat, inputs[1].shape[1::-1], flags=cv2.INTER_LINEAR
        )
        return inputs, target
